 ...
 * See details https://en.How trigonometric functions can be used in CSS?
 *
 * For animation, blueprints, etc.
 *
 * To satisfy your maths teacher by using trigonometry in your everyday life
 *
 * For colors mixing
 *
 * There are no Trigonometric functions in CSS wikipedia.org/wiki/Euclidean_vector#Representations
 *
 * @param {number} x1
 * @param {number} y1
 * @param {number} x2
...
function roundToPowerOfTen(num, pow) {
  ...
  const p = num.toString().length - pow;
  return Number(Number(num).toPrecision(p));
}
 ...
 */
function isPrime(n) {
  for (let index = 2; index <= Math.floor(Math.sqrt(n)); index += 1) {
    if (n % index === 0) {
      return false;
    }
  }
...
function getSumOfDigits(num) {
  return `${num}`.split('').reduce((prev, current) => {
...
function isPowerOfTwo(num) {
  const [first, ...rest] = Number(num).toString(2).split('');
...
function getCountOfOddNumbers(number) {
  return [...Array(Math.abs(number)).keys()].filter((n) => n % 2 === 0).length;
}
